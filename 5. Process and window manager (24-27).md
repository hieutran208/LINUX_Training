...
- Nguyên lý hoạt động:
  ...
  - Tiến trình nhận tín hiệu: Tiến trình sẽ nhận tín hiệu từ kernel: nếu là SIGTERM thì có thể nhận tín hiệu này và có thể xử lý nó, nếu là SIGKILL (tín hiệu không thể bị chặn hoặc xử lý) thì nó sẽ ngay lập tức bị kết thúc mà không có cơ hội để xử lý hay giải phóng tài nguyên.
...
- Load Average (trung bình tải) **là tỷ lệ số lượng tiến trình đang chờ đợi hoặc đang sử dụng CPU** thể hiện mức độ bận rộn của CPU và hệ thống trong một khoảng thời gian nhất định. Khi sử dụng câu lệnh top/htop để theo dõi tiến trình, load average được hiển thị dưới dạng 3 số liên tiếp nhau, biểu thị tải trong lần lượt 1, 5 và 15 phút vừa qua
- Ý nghĩa thông số:
  ...
  - Load Average > Số core CPU: Hệ thống có thể bị quá tải, nếu tiến trình cần CPU nhiều hơn số lượng core có sẵn, sẽ có tiến trình phải chờ đợi hoặc được trì hoãn.

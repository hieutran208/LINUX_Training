...
- Quyền sở hữu: Mỗi folder hay file bất ký đều chịu sự quản lý của 4 đối tượng:
  ...
  - Đối tượng g: group sở hữu file/folder (những user trong nhóm này sẽ có quyền truy cập như nhau)
...
- Cú pháp: chmod + [nhóm người dùng] + [thao tác] + [quyền hạn] + [tên tập tin/thư mục]
...
- Trong phương thức này, quyền được biểu diễn dưới dạng số. Mỗi quyền có một giá trị số nhất định:
  - r (read) = 4
  - w (write) = 2
  - x (execute) = 1
  - (no permission) = 0
- Cú pháp: chmod + [options] + [giá trị định quyền] + [tên file/thư mục]

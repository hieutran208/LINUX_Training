...
- Nguyên lí hoạt động:
  ...
  - Đọc dữ liệu: Khi đọc dữ liệu, các khối dữ liệu nằm trong các ổ đĩa được truy xuất song song.
...
- Nguyên lí hoạt động:
  - Ghi dữ liệu:
    - Chia dữ liệu thành các khối và ghi theo chu kì (mỗi chu kì sẽ ghi 2 khối dữ liệu + 1 parity được tính toán khi ghi dữ liệu, 3 khối trên được ghi xen kẽ vào các ổ trong mảng raid). Các chu kì lặp lại theo vòng tròn đến khi tất cả block dữ liệu và parity được ghi đầy đủ vào tất cả các ổ đĩa trong mảng. Nếu một ổ bị hỏng, có thể khôi phục dữ liệu từ thông tin parity của các ổ còn lại
    - Parity: 
   
- VÍ DỤ: Giả sử có ba ổ A, B, C.
  - Ghi dữ liệu
      - Tệp dữ liệu được chia thành các block và các block này sẽ được ghi lần lượt vào các ổ đĩa.
      - Block 1 có thể được ghi vào ổ đĩa A, Block 2 vào ổ đĩa B, Block 3 vào ổ đĩa C, và tiếp tục chu trình này.
